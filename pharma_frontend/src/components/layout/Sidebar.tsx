    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between gap-3">      
          ...
          <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">v1.0.0</span>
        </div>
      </div>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link to={item.path} key={item.path}>
                      <div className={`flex items-center justify-between px-3 py-2.5 rounded-[6px] transition-all group ${
                        isActive ? 'bg-gray-100 text-black' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
                      }`}>
                        <div className="flex items-center gap-3">
                          <item.icon size={18} strokeWidth={isActive ? 2.5 : 2} className={isActive ? 'text-black' : 'text-gray-400 group-hover:text-gray-600'} />
                          <span className={`text-sm font-medium`}>
                            {item.name}
                          </span>
                        </div>
                        {item.name === 'Medicaments' && (
                           <ChevronRight size={14} className={isActive ? 'text-black' : 'text-gray-300'} />
                        )}
                      </div>
                    </Link>
                  );
      ...
      <div className="p-5 border-t border-[#f0f0f0]">
        <div className="flex items-center gap-3 p-2.5 mb-4 bg-gray-50 border border-gray-50 rounded">
          ...
          <div className="flex-grow min-w-0">
            <p className="text-sm font-semibold text-gray-900 truncate">Mohamed boukab</p>
            <p className="text-[11px] text-gray-500 truncate">pharma@mail.com</p>
          </div>
